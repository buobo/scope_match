...
def _colorize_tagged_line_visible_braces(tagged_line, brace_specs):
    ...
    while i < n:
        old_i = i
        new_i = _skip_color_or_hidden(raw, i, out=out)
        ...
        if ch in ESC_TAGS:
            if i + 1 < n:
                ...
                if plain_idx < len(plain) and esc_ch == plain[plain_idx]:
                    ...
                    if key in wanted:
                        out.append(ida_lines.COLSTR(esc_ch, _brace_scolor_value_for_index(wanted[key])))
                    else:
        ...
        if plain_idx < len(plain) and ch == plain[plain_idx]:
            key = (plain_idx, ch)
            if key in wanted:
                out.append(ida_lines.COLSTR(ch, _brace_scolor_value_for_index(wanted[key])))
            else:
                out.append(ch)
            plain_idx += 1
            i += 1
...
class StickyOverlay(QtWidgets.QWidget):
    ...
    def _schedule_jump_from_pos(self, pos):
        ...
        def _do_jump_request(m=manager, w=parent, line=target_line, row=row_index):
            try:
        ...
        try:
            ...
            QtCore.QTimer.singleShot(
                0,
                lambda m=manager, w=parent, line=target_line, row=row_index: m.jump_to_sticky_target_line(w, line, row),
            )
        return True
    ...
    def paintEvent(self, event):
        ...
        for idx, scope in enumerate(self.scopes):
            ...
            text_rect = QtCore.QRect(
                row_x + TEXT_LEFT_PADDING,
                row_y,
                row_w - TEXT_LEFT_PADDING - TEXT_RIGHT_PADDING,
                row_h,
            )
...
class GutterLineOverlay(QtWidgets.QWidget):
    ...
    def paintEvent(self, event):
        ...
        for idx, scope in enumerate(self.scopes):
            ...
            rect = QtCore.QRect(
                LINE_NO_OVERLAY_LEFT_PAD,
                row_y,
                max(1, self.width() - LINE_NO_OVERLAY_LEFT_PAD - LINE_NO_OVERLAY_RIGHT_PAD),
                row_h,
            )
...
class ScopeStickyManager:
    ...
    def _get_overlay(self, qt_widget):
        ...
        if overlay is None:
            overlay = StickyOverlay(qt_widget, self)
    ...
    def _get_gutter_overlay(self, host_widget, qt_widget):
        ...
        if recreate:
            ...
            overlay = GutterLineOverlay(host_widget)
    ...
    def _jump_custom_viewer_to_line(self, twidget, line_no):
        if not twidget:
            return False
        ...
        for args in (
            (twidget, target_place, 0, 0),
            (twidget, target_place, 0, int(line_no)),
        ):
            try:
    ...
    def jump_to_sticky_target_line(self, qt_widget, line_no, clicked_row=None, record_back=True, jump_reason="sticky"):
        ...
        try:
            ...
            twidget = self.qt_to_twidget.get(id(qt_widget))
            if not twidget:
                try:
            ...
            jump_ok = self._jump_custom_viewer_to_line(twidget, line_no)
            ...
            scroll_ok = self._scroll_qt_view_to_line(qt_widget, line_no, total_lines, covered_rows)
            ...
            try:
                QtCore.QTimer.singleShot(50, lambda w=qt_widget, ln=line_no, tl=total_lines, cr=covered_rows: self._scroll_qt_view_to_line(w, ln, tl, cr))
            except Exception:
                pass
...
class ScopeStickyPlugmod(ida_idaapi.plugmod_t):
    def __init__(self):
        ...
        print("[%s] loaded %s, Qt=%s" % (PLUGIN_NAME, PLUGIN_VERSION, QT_BINDING))
